...
$(document).ready(function () {
    ...
    function getApiDetail() {
        getRequest(
            ...
            function (res) {
                ...
                returnList=res.content.returnList;
    ...
    function renderApiDetail(detail){
        ...
        requestList.forEach(function(log){
            startStr+='<div class="form-group" ><label class="col-sm-2 control-label" style="width:20%;">'+log.name+'</label>'+'<div class="col-sm-10" style="width:70%;">'
                    +'<select id="'+log.dpid+'" class="add-selection">'+'<option value="0">请选择属性</option><option value="12">test1</option><option value="13">test2</option>'
                    +'</select></div></div>';
        });
        $('#start-sec').html(startStr);
    ...
    function getUsedProduct() {
            getRequest(
                '/dataSource/getAllDataSource?page=1',
                function (res) {
                    renderUsedProductList(res.content);
                },
                function (error) {
                    alert(error);
                });
        }
    function renderUsedProductList(list) {
        $('#datasource-sec').empty();
        var productStr='<option value="0">请选择产品</option>';
        //var rechargeDomStr = '';
        list.forEach(function (log) {
            productStr +=
                "<option value='"+log.did+"'>"+log.pname+"</option>";
        });
        $('#datasource-sec').html(productStr);
    }

    $('#datasource-sec').change(function () {
            var ddid=$('#datasource-sec').val();
            getAttrList(ddid);
        })

     function getAttrList(ddid) {
             getRequest(
                 '/dataSource/getAttrList?did='+ddid,
                 function (res) {
     ...
     function renderAttrList(list) {
             ...
             //var dataSourceStr='';
             attrNum=list.length;
             var attrStr='<option value="0">请选择属性</option>'
             if(list==null){
                 return false;
             }
             list.forEach(function (log) {
                attrStr+='<option value='+log.aid+'>'+log.aname+'</option>'
             });
             $('.add-selection').html(attrStr);
         }

     $('#connect-check-btn').click(function () {
             var rightExampleStr='{';
             var i=0;
             ...
             var fin='<p style="width:50px">'+rightExampleStr+'</p>'
             $("#con-res").text(rightExampleStr);
         })
